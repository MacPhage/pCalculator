...
public class Calculator
{
	...
	static String version = "1.2.0, last updated July 27, 2014.";
	...
	Version ver = new Version("MacPhage","pCalculator","v1.2.0");
	public static Config config = new Config("pCalculator");

	public static void main(String args[])
	{

		VariableManager.setConstants();
		...
		while(true)
		{
			if(debug == true)
			{
				...
				else if(baton[0].equals("reload"))
				{
					config.getConfig();
				}
				else if(baton[0].equals("write"))
				{
					config.writeConfig();
				}
				else if(baton[0].equals("delete"))
				{
					try
					{
						File fileTemp = new File(config.getFileName());
						if (fileTemp.exists())
						{
							fileTemp.delete();
						}
			...
			input[0] = JOptionPane.showInputDialog(null,"Enter a command: ",config.get("DefaultCommand"));
			input = input[0].split(" ");
			command = input[0];
			...
			*/
			...
			else if(command.equals("debug"))
			{
				try
				{
					if(baton[0].equals("reset"))
					{
						Debug.alert("Debugging file ("+config.getFileName()+") will be reset to its default state.","warning");
					}
					else if(baton[0].equals("reload"))
					{
						Debug.alert("Debugging file ("+config.getFileName()+") will be reloaded into memory.","warning");
					}
					else if(baton[0].equals("delete"))
					{
						Debug.alert("Debugging file ("+config.getFileName()+") will be deleted.","warning");
					}
					else if(baton[0].equals("write"))
					{
						Debug.alert("Debugging file ("+config.getFileName()+") will written.","warning");
					}
					else
					{
						Debug.alert("No debugging options set. Use \'debug reset\'","warning");
					}
					debug = true;
				}
				catch (Exception e)
